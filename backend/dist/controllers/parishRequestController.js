...
const getRequestById = async (req, res) => {
    try {
        const request = await parishRequestService.getById(req.params.id);
        if (!request) {
...
const updateRequest = async (req, res) => {
    try {
        ...
        const request = await parishRequestService.update(req.params.id, req.body);
        if (!request) {
            res.status(404).json({ success: false, message: "Not found" });
            return;
        }
...
const searchRequests = async (req, res) => {
    try {
        const { data, totalCount, page, limit } = await parishRequestService.search(req.query);
...
const deleteRequest = async (req, res) => {
    try {
        const deleted = await parishRequestService.remove(req.params.id);
        if (!deleted) {
