...
function NoObjectionForm() {
  ...
  const validateForm = () => {
    ...
    const required = {
      ...
      reason: 'Reason for request is required',
      recipientDetails: 'Recipient details are required',
    };
  ...
  const handleSearch = async (page = 1) => {
    ...
    try {
      ...
      if (searchData.recipientDetails) params.append('recipientDetails', searchData.recipientDetails);
      ...
      const response = await authFetch(`/api/no-objection/search?${params.toString()}`);
  ...
  const handlePrint = () => {
    const printArea = document.getElementById('noc-certificate-print-area');
    if (!printArea) return;
    ...
    printWindow.document.write('<html><head><title>No Objection Certificate</title>');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        showToast(editingId ? 'No objection certificate updated successfully.' : 'No objection certificate added successfully.', 'success');
        if (cameFromSearch) {
          setFormData(initialFormData);
          setEditingId(null);
          setErrors({});
  ...
  return (
    <div style={{ width: '100%', padding: '24px 16px', minWidth: 0, overflow: 'hidden' }}>
      ...
      <div className="objection-card">
        ...
        <div className="objection-card-header">
          <h2 className="no-print" style={{ fontSize: '20px', fontWeight: 700, color: '#1f2937', margin: 0 }}>
            {view === 'form'
              ? (editingId ? 'Edit No Objection Certificate' : 'No Objection Certificate Form')
              : view === 'certificate'
          ...
          <div className="no-print" style={{ display: 'flex', gap: '10px' }}>
            {view === 'search' ? (
              ...
              <button className="form-btn objection-view-btn" onClick={handleBackToSearch}>Back to Search</button>
            ) : (
              <>
                <button className="form-btn objection-view-btn" onClick={handleSearchClick}>Search</button>
        ...
        {view === 'certificate' && certificateRecord ? (
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="objection-grid">
              ...
              {field('reason', 'Reason for Request', 'text', true, 'e.g., Marriage in another parish')}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', padding: '16px 24px', borderTop: '1px solid #e5e7eb' }}>
              ...
              <button className="form-btn" type="submit" disabled={loading} style={{ backgroundColor: '#3B5EC2', color: '#fff', border: 'none', borderRadius: '6px', padding: '10px 24px', fontSize: '14px', fontWeight: 500, cursor: 'pointer', width: 'auto', opacity: loading ? 0.7 : 1 }}>
          ...
          <div style={{ padding: '24px 28px' }}>
            <div className="objection-grid" key={resetKey} style={{ marginBottom: '20px' }}>
              ...
              {searchField('placeOfBirth', 'Place of Birth')}
              {searchField('reason', 'Reason')}
              {searchField('recipientDetails', 'Recipient Details')}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', padding: '16px 0', borderTop: '1px solid #e5e7eb' }}>
              <button className="form-btn" type="button" onClick={handleSearchReset} style={{ backgroundColor: '#f3f4f6', color: '#374151', border: '1px solid #d1d5db', borderRadius: '6px', padding: '10px 28px', fontSize: '14px', fontWeight: 500, cursor: 'pointer' }}
                ...
                onMouseLeave={(e) => { e.target.style.backgroundColor = '#f3f4f6'; e.target.style.color = '#374151'; e.target.style.borderColor = '#d1d5db'; }}
              >Reset</button>
              <button className="form-btn" type="button" onClick={handleSearch} disabled={searchLoading} style={{ backgroundColor: '#3B5EC2', color: '#fff', border: 'none', borderRadius: '6px', padding: '10px 28px', fontSize: '14px', fontWeight: 500, cursor: 'pointer', opacity: searchLoading ? 0.7 : 1 }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                      <thead>
                        <tr style={{ backgroundColor: '#f9fafb', borderBottom: '2px solid #e5e7eb' }}>
                          ...
                          <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 600, color: '#374151', whiteSpace: 'nowrap' }}>Recipient</th>
                      ...
                      <tbody>
                        {searchResults.map((r, i) => (
                          <tr key={r.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                            ...
                            <td style={{ padding: '12px 16px', color: '#111827', whiteSpace: 'nowrap' }}>{r.reason}</td>
                            <td style={{ padding: '12px 16px', color: '#111827', whiteSpace: 'nowrap' }}>{r.recipientDetails}</td>
