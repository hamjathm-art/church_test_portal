      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-[0_4px_20px_rgba(30,58,138,0.18)]" style={{ maxWidth: '1100px' }}>
        ...
        <div className="family-card-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 28px', borderBottom: '1px solid #e5e7eb', backgroundColor: '#f9fafb', borderRadius: '12px 12px 0 0' }}>
          ...
          <div className="family-header-btns" style={{ display: 'flex', gap: '10px' }}>
            {view === 'search' ? (
              <ActionButton onClick={handleNewClick}><span style={{ fontSize: '18px', fontWeight: 700, lineHeight: '1' }}>+</span> New Registration</ActionButton>
            ) : view === 'detail' ? (
        ...
        {view === 'detail' && detailRecord ? (
          ...
          <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="family-grid">
                ...
                <div>
                  ...
                  <div className="datepicker-wrapper">
                    <DatePicker
                      selected={(() => {
                        if (formData.sinceYear) {
                          const monthNames = ['January','February','March','April','May','June','July','August','September','October','November','December'];
                          const mIdx = formData.sinceMonth ? monthNames.indexOf(formData.sinceMonth) : 0;
                          return new Date(parseInt(formData.sinceYear), mIdx >= 0 ? mIdx : 0, 1);
                        }
                        return null;
                      })()}
                      onChange={(date) => {
                        if (!date) {
                          setFormData(prev => ({ ...prev, sinceMonth: '', sinceYear: '' }));
                        } else {
                          const monthNames = ['January','February','March','April','May','June','July','August','September','October','November','December'];
                          setFormData(prev => ({ ...prev, sinceMonth: monthNames[date.getMonth()], sinceYear: String(date.getFullYear()) }));
                        }
                      }}
            ...
            <div className="family-bottom-btns flex justify-end gap-3 px-6 py-3 border-t border-gray-200">
              ...
              <ActionButton type="submit" disabled={loading}>{loading ? 'Submitting...' : (editingId ? 'Update' : 'Submit')}</ActionButton>
            </div>
          </form>
        ) : (
          /* Search View */
          <div className="family-search-section" style={{ padding: '24px 28px' }}>
            ...
            <div className="family-search-btns" style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', padding: '16px 0', borderTop: '1px solid #e5e7eb' }}>
              ...
              <SearchButton onClick={() => handleSearch()} disabled={searchLoading}>{searchLoading ? 'Searching...' : 'Search Family'}</SearchButton>
            </div>
