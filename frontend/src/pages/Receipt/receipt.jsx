...
const towardsOptions = [
  { value: 'MassIntentions', label: 'Mass Intentions' },
  { value: 'Donation', label: 'Donation' },
  { value: 'CertificateFee', label: 'Certificate Fee' },
...
const ReceiptForm = () => {
  ...
  const handleSearch = async (page = 1) => {
    const hasSearchCriteria = searchData.receiptNo || searchData.familyCardNo ||
      searchData.towards || searchData.dateOfReceiptFrom || searchData.dateOfReceiptTo;
    ...
    try {
      ...
      if (searchData.receiptNo) params.append('receiptNo', searchData.receiptNo);
      if (searchData.dateOfReceiptFrom) params.append('dateOfReceiptFrom', searchData.dateOfReceiptFrom);
      if (searchData.dateOfReceiptTo) params.append('dateOfReceiptTo', searchData.dateOfReceiptTo);
      if (searchData.towards) params.append('towards', searchData.towards);
      if (searchData.familyCardNo) params.append('familyCardNo', searchData.familyCardNo);
      ...
      const response = await authFetch(`/api/receipt/search?${params.toString()}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const url = editingId ? `/api/receipt/${editingId}` : '/api/receipt';
      ...
      const result = await response.json();
      if (result.success) {
        showToast(editingId ? 'Receipt updated successfully.' : 'Receipt submitted successfully.', 'success');
        if (cameFromSearch) {
          setFormData(initialFormData);
          setEditingId(null);
          setErrors({});
  ...
  const getTowardsLabel = (value) => {
    const opt = towardsOptions.find(o => o.value === value);
        ...
        {view === 'detail' && detailRecord ? (
              ...
              <table className="receipt-detail-table" style={{ margin: '0 auto', borderCollapse: 'collapse', fontSize: '13px', fontFamily: "'Times New Roman', Times, serif", width: '100%' }}>
                <tbody>
                  {[
                    ...
                    { no: 2, label: 'Date of Receipt', value: formatDate(detailRecord.dateOfReceipt) },
                    { no: 3, label: 'Received From', value: detailRecord.receivedFrom },
                    { no: 4, label: 'Family Card No', value: detailRecord.familyCardNo },
                    { no: 5, label: 'Amount', value: detailRecord.amount },
                    { no: 6, label: 'Towards', value: getTowardsLabel(detailRecord.towards) },
                    { no: 7, label: 'Details', value: detailRecord.details },
                  ].map((item) => (
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="receipt-form-section pt-5 px-7 pb-2 border-b border-gray-200">
              <div className="receipt-grid">
                ...
                {field('amount', 'Amount')}
                {selectField('towards', 'Towards', towardsOptions)}
              </div>
            </div>
          ...
          <div className="receipt-search-section" style={{ padding: '24px 28px' }}>
            <div className="receipt-grid" key={resetKey} style={{ marginBottom: '20px' }}>
              ...
              {searchField('dateOfReceiptTo', 'Date of Receipt (To)', 'date')}
              <div>
                <label style={{ display: 'block', fontSize: '14px', fontWeight: 600, color: '#374151', marginBottom: '8px' }}>Towards</label>
                <select name="towards" value={searchData.towards} onChange={handleSearchChange} style={{ ...selectStyle, width: '100%' }}>
              ...
              {searchField('familyCardNo', 'Family Card No')}
                      ...
                      <tbody>
                        {searchResults.map((r, i) => (
                          <tr key={r.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                            ...
                            <td style={{ padding: '10px 12px', color: '#111827', whiteSpace: 'nowrap' }}>{r.receiptNo || '-'}</td>
                            <td style={{ padding: '10px 12px', color: '#111827', whiteSpace: 'nowrap' }}>{r.dateOfReceipt || '-'}</td>
                            <td style={{ padding: '10px 12px', color: '#111827', fontWeight: 500, whiteSpace: 'nowrap' }}>{r.receivedFrom || '-'}</td>
                            <td style={{ padding: '10px 12px', color: '#111827', whiteSpace: 'nowrap' }}>{r.familyCardNo || '-'}</td>
                            <td style={{ padding: '10px 12px', color: '#111827', whiteSpace: 'nowrap' }}>{r.amount || '-'}</td>
                            <td style={{ padding: '10px 12px', color: '#111827', whiteSpace: 'nowrap' }}>{r.towards ? getTowardsLabel(r.towards) : '-'}</td>
